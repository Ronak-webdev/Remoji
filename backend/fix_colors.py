...
def fix_colors():
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            if mask.sum() > 10:
                # Use MEDIAN instead of MEAN to ignore outlines/shadows!
                r_med = float(np.median(arr[:, :, 0][mask]))
                g_med = float(np.median(arr[:, :, 1][mask]))
                b_med = float(np.median(arr[:, :, 2][mask]))
                
                L, a_val, b_val = rgb_to_lab_single(r_med, g_med, b_med)
                
                df.at[idx, 'r'] = round(r_med)
                df.at[idx, 'g'] = round(g_med)
                df.at[idx, 'b'] = round(b_med)
    ...
    print("Fixed CSV colors using Median!")
